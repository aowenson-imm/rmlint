...
static gboolean rm_cmd_parse_limit_sizes(_UNUSED const char *option_name,
    ...
    if(modifier != NULL) {
        if(g_ascii_strcasecmp(modifier, "dirs") == 0 || g_ascii_strcasecmp(modifier, "d") == 0) {
...
#define FUNC(name) ((GOptionArgFunc)rm_cmd_parse_##name)
    ...
    */
    ...
    const GOptionEntry main_option_entries[] = {
        ...
        {"size"             , 's' , 0        , G_OPTION_ARG_CALLBACK , FUNC(limit_sizes)    , _("Specify size limits (optional suffix ,d or ,dirs to apply limits to folders)") , "m-M[,d|,dirs]"} ,
        ...
        {"timeout"          , 0   , 0        , G_OPTION_ARG_INT      , &cfg->trav_timeout   , _("Skip slow/dead folder after timeout seconds") , "N"}                 ,
