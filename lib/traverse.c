...
static void rm_traverse_directory(RmTravBuffer *buffer, RmTravSession *trav_session) {
    ...
    /* Snapshot the command-line root device for cross-device checks in both modes. */
    dev_t root_dev = buffer->stat_buf.st_dev;
    ...
    /* Single traversal loop: only entry transport differs between modes. */
        ...
        if(cfg->ignore_hidden && p->fts_level > 0 && entry_name[0] == '.') {
            ...
            case FTS_D: /* preorder directory */
                if(cfg->limits_specified && cfg->apply_limits_to_dirs && cfg->minsize > 0) {
                    RmOff dir_size = (RmOff)p->fts_statp->st_size;
                ...
                if(cfg->depth != 0 && p->fts_level >= cfg->depth) {
                    ...
                    rm_log_debug_line("Not descending into %s because max depth reached",
                                      p->fts_path);
                } else if(!(cfg->crossdev) && p->fts_dev != root_dev) {
